...
- **FFB Wheel** — range, maxtorque, fxratio, axis effects (idlespring, damper, inertia, friction, **electronic end-stop with separate spring `esgain` + damper `esdamp`**, slew, expo), plus **independent direction inversion** — `axis.invert` flips only the reported HID position (steering direction seen by the game) and `axis.ffbinvert` flips only the FFB torque (force direction), each with its own toggle button
...
✅ **Independent axis & FFB inversion** — `axis.invert` (HID position only) and `axis.ffbinvert` (FFB torque only) are now separate, each persisted to EEPROM with its own toggle button (previously a single invert flipped both at once)
